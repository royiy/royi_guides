...
Terraform הפך לכלי סטנדרטי בתפקידי DevOps, Cloud Engineer, SRE ו-Platform Engineer.
מהיסודות ועד תרחישים מורכבים של ניהול state, מודולים ו-CI/CD.
...
2. **Plan** - מציג "תוכנית" - מה ישתנה (Create/Update/Destroy)
...
- **הרצת `terraform apply` בלי לקרוא את ה-`plan`** -
 זה כמו לעשות `git push --force` בלי להסתכל על מה שדוחפים.
